...
async def run():
    async with async_playwright() as p:
        ...
        await page.fill("#float_createEditPanel__binderName", "UploadDocumentBinderkrishna")
        await page.click("#float_createEditPanel__BinderSaveCancel__saveBtn")
        await page.wait_for_timeout(10000)

        # Step 5: Validate binder creation
        ...
        try:
            cookie_btn = await page.wait_for_selector(
                "xpath=(//button[contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'accept') or contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'cookie')])[1]",
                timeout=3000
            )
            if await cookie_btn.is_visible():
                await cookie_btn.click()
                print("✅ Cookies accepted after upload modal opened.")
        except:
            print("ℹ️ No cookie popup appeared after upload modal.")

        # Scroll inside all scrollable containers (including modal)
        for _ in range(5):
            await page.evaluate("""
