...
async def generate_TearsheetReport_from_tearsheet():
    async with async_playwright() as p:
        ...
        if await tearsheet_locator.is_visible():
            ...
            return
        # Step 7: Check if the 'Tearsheet Report' button is visible
        tearsheet_locator = page.locator("#CompanyHeaderInfo_TearSheetReport_ReportImage > div > img")
        if await tearsheet_locator.is_visible():
            print("✅ 'Tearsheet Report' button is visible.")
            await handle_cookie_popup(page)
            await page.wait_for_timeout(2000)

            # Step 8: Click on the 'Tearsheet Report' button
            await tearsheet_locator.click()
            await page.wait_for_timeout(5000)
            print("✅ 'Tearsheet Report' button clicked.")
            ...
            await browser.close()
    # Run the coroutine
asyncio.run(generate_TearsheetReport_from_tearsheet())
